...
## Unreal Reflection / VTable Analysis – Vorprüfung und Tool-Bedarf

### Fragestellung dieser Phase

Kann die 1047-eigene Online-/Session-Schicht (`FSessions1047::JoinSession`, `FAuth1047::Login`, `FAuth1047Developer`, `OnlineServices1047DeveloperUser`) über Unreal-Reflection-Strukturen (`UClass`/`UFunction`) oder C++-VTables/RTTI rekonstruiert werden, um Aufrufbeziehungen zu Maverick, Steam, EOS oder direkter P2P-Verbindung zu klären?

### Method

Ausschließlich bereits vorhandene Mittel (kein neues Tool installiert): erneute, gezielte `grep`-Auswertung der bereits extrahierten ASCII- und UTF-16LE-`strings`-Dumps der Shipping-Exe nach RTTI-Typenamen (MSVC-Mangling-Präfixe `.?AV`/`.?AU`) und nach UE-Reflection-/Factory-Symbolen. Kein Ghidra-Neuanalyse-Lauf (wie von der Aufgabenstellung gefordert, keine „lange Blindanalyse").

### VERIFIED

- **RTTI (MSVC TypeDescriptor-Namen) existiert für keine der vier Zielklassen** (`FSessions1047`, `FAuth1047`, `FAuth1047Developer`, `OnlineServices1047DeveloperUser`) und für **keine** andere `1047`-präfigierte oder projekteigene Klasse: 0 Treffer für das Muster `.?A[VU][A-Za-z_]*1047` in beiden String-Dumps.
- **RTTI ist im Binary nicht generell deaktiviert, sondern selektiv nur für einzelne Drittanbieter-Bibliotheken aktiv** (2174 RTTI-Typdescriptor-Strings insgesamt gefunden). Beispiele der tatsächlich vorhandenen RTTI-Typen: `std::default_delete<...>`-Instanziierungen aus **grpc** (`AresDNSResolver`, `NativeDNSResolver`, `EventEngine`), Distanzfunktions-Templates aus einer Fuzzy-Matching-Bibliothek (`rl4`-Namespace), `IModuleInterface`-Delegate-Boilerplate, sowie `TCppStructOps<FMatrix2D/3D>`-Templates. **Kein einziger** dieser RTTI-Treffer gehört zu Gameplay-, Online- oder projekteigenem Code – das deckt sich mit der aus Phase 3 bekannten Beobachtung (0 RTTI-Treffer für `PortalWars*`/`GameMode`/`GameState`-Klassen) und bestätigt: **UE-Standardverhalten `bUseRTTI=false` gilt für das gesamte Engine- und Spielcode-Modul**, nur einzelne mitgelinkte Drittanbieter-Bibliotheken wurden separat mit aktiviertem RTTI kompiliert.
- **Factory-/Registrierungssymbole weiterhin nicht auffindbar:** `IOnlineServicesFactory`, `OnlineServicesFactory`, `CreateOnlineServices`, `FOnlineServicesRegistry`, `OnlineServicesRegistry` – 0 Treffer in ASCII **und** UTF-16LE (damit wird der ASCII-only-Befund aus Phase 5 bestätigt, nicht nur wiederholt). Zwei zunächst vielversprechende Treffer (`GetSessionsInterface`, `GetAuthInterface`) erwiesen sich bei Prüfung des exakten Kontexts als **falsche Treffer** – sie gehören zu den bereits bekannten EOS-SDK-Exportfunktionen `EOS_Platform_GetSessionsInterface`/`EOS_Platform_GetAuthInterface`, nicht zu einer 1047-eigenen Factory-Methode.
- **Korrektur einer eigenen methodischen Fehlannahme:** Ein erster Versuch, UObject-Reflection-Maschinerie über die Literal-Strings `Z_Construct_UClass_`, `ProcessEvent`, `FindFunction` nachzuweisen, lieferte 0 Treffer – das ist jedoch **kein aussagekräftiges Ergebnis**, da diese Bezeichner reine C++-Funktionsnamen sind, die in einem Shipping-Build ohne PDB grundsätzlich nicht als eingebettete String-Literale vorliegen (anders als Klassennamen, die über das FName-/Reflection-System zur Laufzeit als Strings benötigt werden, z. B. `OnlineServices1047DeveloperUser`). Dieser Befund wird daher **nicht** als Beleg gegen UObject-Reflection gewertet.

### STRONGLY INDICATED

- Da `OnlineServices1047DeveloperUser` bereits in einer früheren Phase über einen echten, UnrealHeaderTool-generierten Package-Pfad (`/Script/OnlineServices1047Developer`) verifiziert wurde, **ist diese eine Klasse mit hoher Sicherheit ein echter reflektierter UObject-Typ** (UCLASS/USTRUCT). Für `FAuth1047`, `FAuth1047Developer` und `FSessions1047` selbst existiert **kein** entsprechender `/Script/`-Pfad-Beleg – das deckt sich mit dem öffentlich bekannten Architekturmuster von Epics `UE::Online`-Modul (`OnlineServicesInterface`-Plugin), in dem `FAuthCommon`/`FSessionsCommon`-abgeleitete Interface-Implementierungen als reine, nicht-UObject-basierte C++-Klassen (über ein `TOnlineComponent`-Registrierungsmuster) implementiert sind – nicht über das klassische `UCLASS`/`UFUNCTION`-Reflection-System. Diese Einordnung stützt sich auf öffentlich dokumentierte UE5-Engine-Architektur (Hintergrundwissen), **nicht** auf einen Fund in diesem spezifischen Binary, und wird daher als STRONGLY INDICATED, nicht VERIFIED eingestuft.

### Konsequenz: `UClass → UFunction → Native Function Pointer` ist für die Kernfrage voraussichtlich der falsche Analyseansatz

Da `FAuth1047`/`FSessions1047` aller Wahrscheinlichkeit nach **keine** UObject-Klassen sind, liefe eine Suche nach `UClass`/`UFunction`/`FindFunction`/`ProcessEvent`-Aufrufketten für genau diese beiden Zielklassen ins Leere – nicht wegen fehlender Analyse-Tiefe, sondern weil dieser Mechanismus für sie architektonisch nicht zutrifft. Relevant bliebe UClass/UFunction-Reflection höchstens für `OnlineServices1047DeveloperUser`/`UOnlineServices1047DeveloperSettings`.

### Tool-Bedarf – STOPP gemäß Aufgabenstellung

Für den nächsten sinnvollen Schritt (VTable-Struktur zu `FAuth1047`/`FSessions1047` identifizieren und mit deren Konstruktor-Code abgleichen, um die Aufrufkette überhaupt zu finden) fehlt ein geeignetes Werkzeug bzw. reicht der aktuelle Ghidra-Analysezustand nachweislich nicht aus:

- **Welches Tool:** IDA Pro (idealerweise mit Hex-Rays-Decompiler für x86-64), alternativ Binary Ninja. Beide sind **nicht** in dieser Umgebung installiert.
- **Warum:** Da RTTI für alle relevanten Klassen fehlt (s. o.), kann eine VTable keiner Klasse automatisch per eingebettetem Typnamen zugeordnet werden – die einzige verbleibende Methode ist, den Konstruktor-Code zu finden, der eine VTable-Adresse in ein neu erzeugtes Objekt schreibt (typisches Muster `mov [reg], offset vtable`), und diesen Code mit den bekannten String-Adressen/Aufrufkontexten in Verbindung zu bringen. Das erfordert **verlässliche Codeabdeckung durch Disassemblierung** – genau das, was in der vorherigen Phase (`Ghidra Xref Recovery / Function Boundary Validation`) für dieses konkrete Ghidra-Projekt eindeutig als unzureichend nachgewiesen wurde (0 von 12 Code-Xrefs, **einschließlich** der Kontrollgruppe aus garantiert verwendeten generischen UE-Strings). Eine Fortsetzung mit Ghidra im aktuellen Analysezustand würde mit hoher Wahrscheinlichkeit erneut scheitern – das wäre exakt die in der Aufgabenstellung ausgeschlossene „oberflächliche Ersatzanalyse". IDA Pro verfügt über deutlich ausgereiftere Heuristiken speziell für MSVC-VTable-/Konstruktor-Mustererkennung in großen, gestrippten PE-Binaries als Ghidras Standard-Auto-Analyse, was in dieser Größenordnung (245 MB Codesektion) einen relevanten praktischen Unterschied macht.
- **Konkrete Analyse damit:** (1) Automatische/heuristische VTable-Erkennung in `.rdata` (Arrays von Codepointern), (2) Rückverfolgung, an welchen Konstruktor-Adressen diese VTables gesetzt werden, (3) Abgleich dieser Konstruktor-Adressen mit dem Codeumfeld der bekannten String-Adressen (z. B. `0x1503c45e4` für `FSessions1047`) über einen zuverlässigeren Disassemblierungs-/Xref-Mechanismus als den bisher getesteten.
- **Sinnvolle Alternative ohne dieses Tool:** Eine rein strukturelle Heuristik-Suche nach VTable-förmigen Pointer-Arrays in `.rdata` (Arrays aus 8-Byte-Werten, die alle in den `.text`-Adressbereich fallen) wäre mit den vorhandenen Ghidra-Headless-Skripten technisch möglich, würde aber **ohne Namenszuordnung** (da keine RTTI vorhanden ist) nur eine unspezifische Liste "irgendwelcher VTables" liefern, ohne dass sich diese verlässlich einer der vier Zielklassen zuordnen ließen – das wäre spekulativ und wird hier bewusst **nicht** durchgeführt, statt einen unbelastbaren Befund zu präsentieren.

**Diese Phase wird an dieser Stelle angehalten**, bis eine Entscheidung über Tool-Beschaffung vorliegt. Abschnitte 4 und 5 der Aufgabenstellung (Maverick-Verbindung, Klassifizierung BACKEND-BOUND/ALTERNATIVE TRANSPORT/HYBRID/UNKNOWN) werden **nicht** auf Basis der bisherigen, unzureichenden Codeabdeckung spekulativ beantwortet – die Klassifizierung bleibt bei **UNKNOWN**, unverändert gegenüber der letzten Phase, bis belastbare Call-Chain-Evidenz vorliegt.
